...
function bool PostAbilityCostPaid(XComGameState_Effect EffectState, XComGameStateContext_Ability AbilityContext, XComGameState_Ability kAbility, XComGameState_Unit SourceUnit, XComGameState_Item AffectWeapon, XComGameState NewGameState, const array<name> PreCostActionPoints, const array<name> PreCostReservePoints)
{
	...
	if (iUsesThisTurn >= class'RPGO_Helper'.static.GetAbilityConfig().GetConfigIntValue("HITANDRUN_USES_PER_TURN"))
		return false;
	...
	if (kAbility.SourceWeapon == EffectState.ApplyEffectParameters.ItemStateObjectRef)
	{
		...
		if (!AbilityState.IsMeleeAbility() && TargetUnit != none)
		{
			if(X2TacticalGameRuleset(XComGameInfo(class'Engine'.static.GetCurrentWorldInfo().Game).GameRuleset).VisibilityMgr.GetVisibilityInfo(SourceUnit.ObjectID, TargetUnit.ObjectID, VisInfo))
			{
				...
				{
					ValidAbilities = class'RPGO_Helper'.static.GetAbilityConfig().GetConfigNameArray("HITANDRUN_ABILITYNAMES");
					...
					if (ValidAbilities.Find(kAbility.GetMyTemplateName()) != -1)
					{
						if (SourceUnit.NumActionPoints() < 2 && PreCostActionPoints.Length > 0)
						{
							...
							if (AbilityState != none)
							{
								...
								if (!class'RPGO_Helper'.static.GetAbilityConfig().GetConfigBoolValue("HITANDRUN_FULLACTION"))
								{
