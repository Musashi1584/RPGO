class X2Effect_DeathFromAboveRPG extends X2Effect_Persistent;
...
function bool PostAbilityCostPaid(XComGameState_Effect EffectState, XComGameStateContext_Ability AbilityContext, XComGameState_Ability kAbility, XComGameState_Unit SourceUnit, XComGameState_Item AffectWeapon, XComGameState NewGameState, const array<name> PreCostActionPoints, const array<name> PreCostReservePoints)
{
	...
	SourceUnit.GetUnitValue('DeathFromAboveRefunds', CountUnitValue);
			...
			if (TargetUnit.IsDead() && PrevTargetUnit != None && SourceUnit.HasHeightAdvantageOver(PrevTargetUnit, true))
			{
				...
				if (SourceUnit.NumActionPoints() == 0 && PreCostActionPoints.Length > 0)
				{
					...
					if (AbilityState != none)
					{
						...
						SourceUnit.SetUnitFloatValue('DeathFromAboveRefunds', CountUnitValue.fValue + 1, eCleanup_BeginTurn);
