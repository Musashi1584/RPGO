...
class RPGO_UIImageSelector extends UIPanel;
...
var array<RPGO_UIClassIcon> ImageChips; 
...
simulated function RPGO_UIImageSelector InitImageSelector(optional name InitName, 
...
{
	local RPGO_UIClassIcon Chip, LeftNavTargetChip, RightNavTargetChip, UpNavTargetChip, DownNavTargetChip; 
	local int iChip, iRow, iCol, iMaxChipsPerRow, iLastRowLength, iOffset; 
	
	Images = _Images;

	iMaxChipsPerRow = int( (width - ITEM_PADDING) / ( class'RPGO_UIClassIcon'.default.iDefaultSize + ITEM_PADDING ) );
	...
	for( iChip = 0; iChip < Images.Length; iChip++ )
	{
		...
		Chip = Spawn( class'RPGO_UIClassIcon', ChipContainer);
		Chip.InitSquadImage( '',
							iChip, 
							Images[iChip], 
							GetChipX(iCol), 
							GetChipY(iRow), 
							class'RPGO_UIClassIcon'.default.iDefaultSize, 
							iRow, 
							iCol,
							OnPreviewColor, 
							OnAcceptColor);
	...
	ChipHeight = GetChipY(iRow) + class'RPGO_UIClassIcon'.default.iDefaultSize; 
...
simulated function SetInitialSelection()
{
	...
	ScrollToRow(RPGO_UIClassIcon(ChipContainer.Navigator.GetSelected()).Row);
}

simulated function float GetChipX( int iCol )
{
	return ((class'RPGO_UIClassIcon'.default.iDefaultSize + ITEM_PADDING) * iCol) + (EDGE_PADDING); 
}

simulated function float GetChipY( int iRow )
{
	return ((class'RPGO_UIClassIcon'.default.iDefaultSize + ITEM_PADDING) * iRow) + (EDGE_PADDING); 
}
