class X2Effect_ReducedRecoveryTime extends X2Effect_Persistent config(RPG);
...
function ApplyFieldSurgeon(XComGameState_Effect EffectState, XComGameState_Unit OrigUnitState, XComGameState NewGameState)
{
	...
	`LOG("Field Surgeon: Source Unit Valid.",, 'RPG');
	...
	`LOG("Field Surgeon: Target Unit Can Be Healed.",, 'RPG');
	...
	`LOG("Field Surgeon: NumEffects=" $ NumEffects $ ", Chance=" $ default.FIELD_SURGEON_CHANCE_FOR_NUM_EFFECTS[NumEffects],, 'RPG');
	if(`SYNC_RAND(100) < default.FIELD_SURGEON_CHANCE_FOR_NUM_EFFECTS[NumEffects])
		bApplyFieldSurgeon = true;

	`LOG("Field Surgeon: ApplyFieldSurgeon=" $ bApplyFieldSurgeon,, 'RPG');

	if(bApplyFieldSurgeon)
	{
		`LOG("Field Surgeon : Pre update LowestHP=" $ UnitState.LowestHP,, 'RPG');
		UnitState.LowestHP += 1;
		`LOG("Field Surgeon : Post update LowestHP=" $ UnitState.LowestHP,, 'RPG');
